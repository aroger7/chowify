...
class PageSelector extends Component {
	constructor(props) {
		...
		this.handleLastPageButtonClick = this.handleLastPageButtonClick.bind(
			this
		);
		this.handleNextPageButtonClick = this.handleNextPageButtonClick.bind(
			this
		);
	...
	render() {
		const startPageNum =
			this.props.currentPage - 2 >= 0 ? this.props.currentPage - 2 : 0;
		const endPageNum =
			this.props.currentPage + 2 < this.props.numPages
		...
		return (
			<PageSelectorWrapper>
				...
				<Button
					key="last"
					content=">>"
					disabled={this.props.numPages < 1 || this.props.currentPage >= endPageNum}
